  ...
  <q-layout view="hHh lpR fFf">
    ...
    <q-drawer v-model="leftDrawerOpen" show-if-above bordered>
      <q-list>
        ...
        <router-link to="/admin/pass" class="router-link-class">
          <q-item clickable target="_blank" rel="noopener">
            <q-item-section avatar>
    ...
    <q-page-container>
      <div class="q-pa-md">
        <q-table :rows="rows" :columns="columns" row-key="id" :pagination="true">
          <template v-slot:body-cell-action="{ row }">
            <q-btn color="positive" class="q-mx-auto" icon="check" @click="approveApplication(row.idNumber)" />
            <q-btn color="negative" class="q-mx-auto " icon="clear" @click="disapproveApplication(row.idNumber)" />
          </template>
          ...
          <!-- TODO：与后端链接，检查效果 -->
          <!-- <q-card-section>
                <div class="text-overline text-orange-9">申请时间：{{ registrationTime }}</div>
              ...
              <q-card-actions>
                <q-btn flat color="dark" label="Accept" class="q-mx-auto" icon="check"
                  @click="approveApplication(row.idNumber)" />
                <q-btn flat color="primary" label="Reject" class=" q-mx-auto " icon=" clear"
                  @click="disapproveApplication(row.idNumber)" />
...
onMounted(() => {
  axiosInstance.post('/shop/showByStatus0').then((response) => {
    const r = response.data['data']
    console.log(r[0]['registrationTime']);
...
function approveApplication(id) {
  // 根据行的id执行同意申请操作
  axiosInstance.put('/shop/approve', { idNumber: id }).then((response) => {
    const r = response.data['data']
    console.log(r);
  });
  axiosInstance.post('/shop/showByStatus0').then((response) => {
...
function disapproveApplication(id) {
  // 根据行的id执行同意申请操作
  axiosInstance.put('/shop/disapprove', { idNumber: id }).then((response) => {
    const r = response.data['data']
    console.log(r);
  });
  axiosInstance.post('/shop/showByStatus0').then((response) => {
